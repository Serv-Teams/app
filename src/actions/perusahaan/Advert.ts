...
export const getAdverts = unstable_cache(
  async () => {
    const db = client.db("corporate");
    const adverts = await db.collection("adverts").find({}).toArray();
    return adverts;
  },
  ["adverts"],
  { revalidate: 1, tags: ["adverts"] }
);

export const getAdvert = async (slug: string) => {
  const db = client.db("corporate");
  const advert = await db.collection("adverts").findOne({ slug: slug });
  return advert;
};

export async function upsertPost(prevState: any, formData: FormData) {
  const db = client.db("blog");
  const posts = db.collection<Advert>("posts");
